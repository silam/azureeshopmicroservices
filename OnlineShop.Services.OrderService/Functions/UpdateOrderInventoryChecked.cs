...
{
    public static class UpdateOrderInventoryChecked
    {
        [FunctionName("UpdateOrderInventoryChecked")]
        public static async Task Run([ServiceBusTrigger("inventorycheckedtopic", "UpdateOrder", AccessRights.Listen,
            Connection = "ServiceBusConnectionString")]string topicMessage, TraceWriter log)
        {
            try
            {
                ...
                if (order.RequiresResponse && order.Status == "Insufficient Inventory")
                {
            ...
            {
                log.Error($"UpdateOrderInventoryChecked function failed. Exception: {ex.Message}");
            }

            log.Info($"UpdateOrderInventoryChecked function finished execution: {topicMessage}");
        }
